...
export default function ProviderNotifications() {
  ...
  const handleTap = (notif) => {
    if (notif.booking_id) {
      navigate(`/provider/booking/${notif.booking_id}`);
    }
  };
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 800 }}>
            ...
            {unreadCount > 0 && (
              <span style={{
                marginLeft: 8, fontSize: 12, fontWeight: 800, padding: '2px 8px',
                borderRadius: 100, background: '#FF5722', color: '#fff', verticalAlign: 'middle',
              }}>{unreadCount}</span>
            )}
          </h1>
          {unreadCount > 0 && (
            <button
              onClick={markAllRead}
              style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: '#FF5722', background: 'none', border: 'none', cursor: 'pointer' }}
            >
        ...
        {notifications.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', background: 'var(--bg-card)', borderRadius: 20, border: '1.5px dashed var(--border-color)', marginTop: 20 }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 13, lineHeight: 1.6 }}>
              New booking requests, reviews, and<br />updates will appear here.
            </p>
          </div>
        ) : (
            ...
            {notifications.map(n => {
              ...
              return (
                ...
                >
                  ...
                  {/* Content */}
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: n.is_read ? 600 : 800, fontSize: 14, color: 'var(--text-primary)', marginBottom: 3 }}>
                      {n.title}
                    </div>
